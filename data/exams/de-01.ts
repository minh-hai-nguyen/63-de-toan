...
b) Từ $BCEF$ nội tiếp: $\widehat{FEB}=\widehat{FCB}$ (cùng chắn $\overparen{BF}$). Trong $(O)$: $\widehat{NMB}=\widehat{FCB}$ (cùng chắn $\overparen{BN}$). Suy ra $\widehat{FEB}=\widehat{NMB}$, ở vị trí đồng vị nên $EF\parallel MN.$
Lại có $\widehat{ABM}=\widehat{ACN}$ nên các cung $\overparen{AM}=\overparen{AN}$, do đó tam giác $MON$ cân tại $O$ với $OA$ là phân giác, suy ra $OA\perp MN.$ Vì $EF\parallel MN$ nên $OA\perp EF.$
